...
Deno.test("E2E: tool round completo (uppercase)", async () => {
  ...
  const call = toolCalls[0] as { type: "tool_call"; tool: string; args: string };
  ...
  assert(
    ...
    `content inclui "PELICANO" (conteúdo: ${result.content.content.slice(0, 80)}...)`,
  );
...
Deno.test("E2E: contexto entre rounds com multi-tool (sum)", async () => {
  ...
  assert(
    result.content.content.includes("15") && result.content.content.includes("42"),
    `content final menciona ambas as somas (15 e 42); conteúdo: ${result.content.content.slice(0, 120)}...`,
  );
  assertEquals(agent.state, "idle");
});