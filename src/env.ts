 ...
 * ```
 */
export async function createRunWorkspace(options: RunWorkspaceOptions): Promise<Workspace> {
 ...
 * ```
 */
export async function appendLineLocked(path: string, line: string): Promise<void> {
  const text = line.endsWith("\n") ? line : `${line}\n`;
  await withFileLock(path, () => Deno.writeTextFile(path, text, { append: true }));
}