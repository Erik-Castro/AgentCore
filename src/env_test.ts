...
Deno.test("env: createWorkspace cria dir 0o700 efêmero; cleanup remove a árvore", rw, async () => {
...
Deno.test("env: createRunWorkspace garante baseDir e cria subdir run 0o700", rw, async () => {
  const base = await createWorkspace({ prefix: "wbase-" });
  try {
    const ws = await createRunWorkspace({ baseDir: base.path, prefix: "run-" });
    try {
      const info = Deno.statSync(ws.path);
      assertEquals(info.isDirectory, true);
      assertEquals((info.mode! & 0o777) >>> 0, 0o700);
...
Deno.test("env: withFileLock executa o corpo e libera o lock no close", rw, async () => {
...
Deno.test("env: appendLineLocked em sequência preserva ordem e newline final", rw, async () => {
