...
function textRound(text: string): Ev[] {
  return [
    { type: "response.reasoning_summary_text.delta", item_id: "rs_1", output_index: 0, summary_index: 0, delta: "p", sequence_number: 1 } as Ev,
    { type: "response.output_text.delta", item_id: "m_1", output_index: 0, content_index: 0, delta: text, sequence_number: 2 } as Ev,
    { type: "response.output_text.done", item_id: "m_1", output_index: 0, content_index: 0, text, sequence_number: 3 } as Ev,
    { type: "response.completed", response: { id: "r", status: "completed", usage: usage(), output: [] }, sequence_number: 4 } as unknown as Ev,
  ];
}
...
function toolRound(name: string, args: string, callId = "call_1"): Ev[] {
  return [
    ...
    { type: "response.completed", response: { id: "r", status: "completed", usage: usage(2, 1), output: [] }, sequence_number: 4 } as unknown as Ev,
  ];
}

function toolRoundMany(calls: Array<{ name: string; args: string; call_id: string }>): Ev[] {
  const events: Ev[] = [];
  calls.forEach((call, index) => {
    ...
    events.push({
      type: "response.output_item.added",
      output_index: index,
      item: { type: "function_call", id: itemId, call_id: call.call_id, name: call.name, arguments: "", status: "in_progress" },
    ...
    events.push({
      type: "response.output_item.done",
      output_index: index,
      item: { type: "function_call", id: itemId, call_id: call.call_id, name: call.name, arguments: call.args, status: "completed" },
      sequence_number: index * 3 + 3,
    } as Ev);
  });
  events.push({
    type: "response.completed",
    response: { id: "r", status: "completed", usage: usage(2, calls.length), output: [] },
    sequence_number: calls.length * 3 + 1,
  } as unknown as Ev);
  return events;
}

async function collect(gen: AsyncGenerator<AgentEvent, TExecutionResult, void>) {
...
function makeAgent(responses: ResponsesCall, options: ConstructorParameters<typeof ReAct>[1] = {}) {
  const agent = new ReAct(
    { model: "model-x", thinking: "low", system_prompt: "sys-prompt", maxRounds: 6 },
    { responses, ...options },
  );
...
Deno.test("round único de texto retorna conteúdo, raciocínio e usage", async () => {
  ...
  assertEquals(events.map((e) => e.type), ["reasoning", "content", "content.done"]);
});
...
Deno.test("self-healing: erro de tool volta ao contexto e o loop continua", async () => {
  ...
  const responses: ResponsesCall = async (request) => {
    requestCount++;
    if (requestCount === 1) return streamOf(toolRound("ferramenta_inexistente", "{}", "call_9"));
    ...
    const content = (systemMessages[0] as { content?: unknown })
      ?.content;
    assertEquals(String(content).includes("Erro na ferramenta \"ferramenta_inexistente\""), true);
  ...
  assertEquals(failed, {
    ...
    ok: false,
    output: 'Ferramenta desconhecida "ferramenta_inexistente". Disponível: uppercase',
  });
  assertEquals(result.rounds, 2);
